...
public class ProductService implements IDao<Product> {
    ...
    private final List<Product> pizzalist = new ArrayList<>();
    ...
    public Product update(Product p){
        for (int i=0; i<pizzalist.size(); i++){
            if (pizzalist.get(i).getId() == p.getId()){
                pizzalist.set(i, p);
                return p;
            }
        }
    ...
    public boolean delete(long id) {
        return pizzalist.removeIf(x -> x.getId() == id);
    }

    @Override
    public Product findById(long id){
        for (Product p:pizzalist){
            if (p.getId() == id){
                return p;
            }
        }
